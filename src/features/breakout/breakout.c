...
void drawScore()
{
    ...
    strcpy(str, "Level ");
    NUMBER_ToDecimal(str + 6, levelCountBreackout, 2, true);
    UI_PrintStringSmallBufferNormal(str, (uint8_t *)gSubMenu_OFF_ON); // reusing buffer?

    // Ball
    // sprintf(str, "Ball %02u", (ballCount < 0) ? 0 : ballCount);
    strcpy(str, "Ball ");
    NUMBER_ToDecimal(str + 5, (ballCount < 0) ? 0 : ballCount, 2, true);
    UI_PrintStringSmallBufferNormal(str, (uint8_t *)gSubMenu_OFF_ON);

    // Score
    // sprintf(str, "Score %04u", score);
    strcpy(str, "Score ");
    NUMBER_ToDecimal(str + 6, score, 4, true);
    UI_PrintStringSmallBufferNormal(str, (uint8_t *)gSubMenu_OFF_ON);
}
